...
if __name__ == "__main__":
    ...
    at = [0, 1, 2, 3, 4, 5]
    priority = [0, 1, 2, 0, 4, 5]
    bt = [8, 4, 2, 1, 3, 2]
    n = len(nameofp)
    p = []
    for i in range(0, len(nameofp)):
        p.append(Process(nameofp[i], at[i], bt[i], priority[i]))

    print("Processes"+" Priority"+" Burst time " +
          " Arrival Time")
    for i in range(n):
        print(" " + str(p[i].name) + "\t\t" + str(p[i].p) + "\t" +
              str(p[i].bt) + "\t\t" +
              str(p[i].at) + "\t ")
    # sort
    for i in range(0, n):
    ...
    while(True):
        ...
        for i in range(0, n):
            for j in range(0, n-i-1):
                if(arrivedprocesses[j].p > arrivedprocesses[j+1].p):
                    temp = arrivedprocesses[j]
                    arrivedprocesses[j] = arrivedprocesses[j+1]
                    arrivedprocesses[j+1] = temp
                elif(arrivedprocesses[j].p == arrivedprocesses[j+1].p):
